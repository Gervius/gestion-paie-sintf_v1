...
import { referentielsSectionsStore, referentielsSectionsIndex } from '@/routes';

export default function Create() {
    const { produits, unitesMesure } = usePage<any>().props;

    const { data, setData, post, processing, errors } = useForm({
        code_section: '', nom_section: '', taux_journalier: '',
    ...
    return (
        <div className="max-w-3xl mx-auto p-6 space-y-6">
            ...
            <Heading title="Nouvelle Section" description="Créer une nouvelle section d'exploitation SINTF" />
            ...
            <form onSubmit={handleSubmit} className="bg-white p-8 rounded-2xl border border-border shadow-sm space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <input type="text" value={data.code_section} onChange={(e) => setData('code_section', e.target.value)} className="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-secondary outline-none" placeholder="Ex: SEC-01" required />
