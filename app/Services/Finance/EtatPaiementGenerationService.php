        ...
        return DB::transaction(function () use ($sectionIds, $dateDebut, $dateFin) {
            ...
            foreach ($sectionIds as $sectionId) {
                ...
                foreach ($lignes as $siteId => $lignesDuSite) {
                    // INTENTION : Suppression du regroupement par type_pointage.
                    // On génère une seule référence et un seul État de Paiement pour le couple Section + Site.
                    ...
                    $etat = EtatPaiement::create([
                        ...
                        'type_pointage' => 'MIXTE', // Signale que l'état fusionne le journalier et le rendement
                        'statut' => 'PROVISOIRE',
                    ]);

                    // Regroupement unique par Agent pour ce Site
                    $lignesParAgent = $lignesDuSite->groupBy('personnel_id');
                    $totalBrutEtat = 0;
                    ...
                    foreach ($lignesParAgent as $personnelId => $groupesLignes) {
                        ...
                        // INTENTION : Création d'un ticket unique par agent contenant la somme de tous ses gains (Journaliers + Rendements)
